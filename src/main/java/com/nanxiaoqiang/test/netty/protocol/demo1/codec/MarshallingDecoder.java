...
public class MarshallingDecoder {
	...
	int objectSize = in.readInt();// 首先是int长度的data长度
	ByteBuf buf = in.slice(in.readerIndex(), objectSize);// 读取Data size大小的数据
	ByteInput input = new ChannelBufferByteInput(buf);
	try {
	    unmarshaller.start(input);
	    Object obj = unmarshaller.readObject();// 使用marshalling转化为Object
	    unmarshaller.finish();
	    in.readerIndex(in.readerIndex() + objectSize);// 移动指针到结尾
	    return obj;
	} finally {
	    unmarshaller.close();
	}
